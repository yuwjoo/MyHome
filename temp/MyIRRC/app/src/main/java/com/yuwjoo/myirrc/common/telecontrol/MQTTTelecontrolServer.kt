 ...
 * MQTT遥控服务
 */
class MQTTTelecontrolServer private constructor() {

    companion object {
        private const val TAG = "MQTTConnect"

        private const val MQTT_SERVER_URI = "tcp://47.115.161.79:1883" // MQTT服务器地址
        private const val MQTT_USERNAME = "my-ir-rc" // 用户名
        private const val MQTT_PASSWORD = "my-ir-rc" // 密码
        private const val MQTT_DEFAULT_QOS = 1 // 默认消息模式

        @Volatile
        private var instance: MQTTTelecontrolServer? = null

        /**
         * 获取单例实例
         * @return mqtt连接类实例
         */
        fun getInstance(): MQTTTelecontrolServer {
            if (instance == null) {
                synchronized(MQTTTelecontrolServer::class.java) {
                    if (instance == null) {
                        instance = MQTTTelecontrolServer()
                    }
                }
            }
            return instance!!
        ...
        mqttAsyncClient.setCallback(object : MqttCallback {
            ...
            override fun messageArrived(topic: String, message: MqttMessage) {
                when (topic) {
                    ...
                    // 卧室空调设备状态改变
                    TelecontrolHelper.TOPIC_DEVICE_BEDROOM_AC -> {
                        val newACDevice = Gson().fromJson(
                            String(message.payload),
                            BedroomAirConditioner.ACDevice::class.java
                        )
                        BedroomAirConditioner.aCDevice = newACDevice
                    }
                }
            }
            ...
            override fun connectComplete(reconnect: Boolean, serverURI: String) {
                subscribe(TelecontrolHelper.TOPIC_RC_BEDROOM_AC)
