package com.yuwjoo.myhome.module.udp

/**
 * UDP 通信配置常量
 */
internal object UdpConfig {
    // 网络
    const val MULTICAST_ADDR = "239.0.0.100" // 组播地址
    const val PORT = 8899 // 通信端口
    const val BUFFER_SIZE = 1024 // 接收缓冲区大小

    // 帧头
    const val MAGIC_H = 0x59.toByte() // 魔数高字节 Y
    const val MAGIC_L = 0x48.toByte() // 魔数低字节 H
    const val VERSION = 0x01.toByte() // 协议版本
    const val HEADER_SIZE = 11 // 帧头字节数
    const val ACK_PAYLOAD_SIZE = 4 // Ack 负载字节数

    // 心跳
    const val HEARTBEAT_INTERVAL = 1_500L // 心跳间隔（毫秒）
    const val HEARTBEAT_OFFLINE_TIMEOUT = 4_500L // 设备离线超时（毫秒）

    // Ack 重传
    const val ACK_BASE_TIMEOUT = 150L // 首次重传等待超时（毫秒）
    const val ACK_MAX_RETRIES = 5 // 最大重传次数
    const val ACK_BACKOFF_MULTIPLIER = 2 // 重传间隔倍增因子

    // 能力前缀
    const val ABILITY_PREFIX_TOPIC = "topic:" // 订阅的主题
    const val ABILITY_PREFIX_SKILL = "skill:" // 设备技能

    // 本机信息
    val deviceName: String = "${android.os.Build.MANUFACTURER}-${android.os.Build.MODEL}" // 本机设备名称
    val deviceAbilities: List<String> = emptyList() // 本机设备能力列表

    // 消息类型
    object Type {
        const val HEARTBEAT = 0x01.toByte() // 设备在线宣告
        const val OFFLINE = 0x02.toByte() // 设备主动离网
        const val CALL = 0x03.toByte() // 向新发现设备发起呼叫
        const val ANSWER = 0x04.toByte() // 回应 Call
        const val ACK = 0x05.toByte() // 消息确认应答
        const val JSON = 0x10.toByte() // JSON 消息
        const val RAW = 0x11.toByte() // 原始二进制消息
    }

    // 标志位
    object Flags {
        const val NONE = 0x00.toByte() // 无标志
        const val ACK_REQUIRED = 0x01.toByte() // 需要回复 Ack
        const val ORDERED = 0x02.toByte() // 有序，校验重复和排序
    }
}
