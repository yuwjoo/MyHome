 ...
 * electron api
 */
export type TElectronApi = TIpcApiShape & {
  /** 页面渲染控制能力 */
  webFrame: WebFrame
  /** 文件等 web 工具能力 */
  webUtils: WebUtils
  /** 订阅主进程推送消息 */
  addListener: <C extends TIpcMsgChannel>(channel: C, listener: TIpcMsgListener<C>) => void
  /** 取消订阅（须传入与 addListener 相同的回调引用） */
  removeListener: <C extends TIpcMsgChannel>(channel: C, listener: TIpcMsgListener<C>) => void
}
