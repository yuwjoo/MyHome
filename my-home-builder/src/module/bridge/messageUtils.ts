...
class MessageUtils {
  ...
  send<G extends ActionGroupName, M extends ActionMessageName<G>>(
    ...
    if (callbacks) {
      for (const [callbackName, handler] of Object.entries<Callback>(
        callbacks as Record<string, Callback>,
      )) {
  ...
  on<G extends EventGroupName, E extends EventMessageName<G>>(
    ...
    const alreadyListening = callbackCenter.hasHandler(
      groupName,
      messageId,
      callbackName,
    );
    ...
    if (alreadyListening) {
      const lastResponse = callbackCenter.getLastResponse(
        groupName,
        messageId,
        callbackName,
      );
  ...
  off<G extends EventGroupName, E extends EventMessageName<G>>(
    ...
    const messageId = eventName;
    const callbackName = "onMessage";
    const stillListening = callbackCenter.hasHandler(
      groupName,
      messageId,
      callbackName,
    );
